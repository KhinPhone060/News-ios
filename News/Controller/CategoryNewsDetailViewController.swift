...
class CategoryNewsDetailViewController: UIViewController {
    ...
    @IBAction func savePressed(_ sender: UIButton) {
        if Auth.auth().currentUser?.uid != nil{
            ...
            db.collection(Constant.FStore.collectionName).whereField(Constant.FStore.newsURL, isEqualTo: categoryNews?.url)
                .getDocuments() { (querySnapshot, err) in
                    if let err = err {
                        ...
                        self.db.collection(Constant.FStore.collectionName)
                            .whereField(Constant.FStore.newsURL, isEqualTo: self.categoryNews?.url)
                            .getDocuments() { (querySnapshot, err) in
                                if let e = err {
    ...
    func loadBookmarkIcon() {
        db.collection(Constant.FStore.collectionName)
            .whereField(Constant.FStore.newsURL, isEqualTo: categoryNews?.url)
